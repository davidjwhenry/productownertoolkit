...
- `prototypes/` for interactive prototypes reviewed in the playground: declarative HTML variants generated by `prototype-builder`, plus exploratory `.pen` files and wireframes
...
The playground's prototype contract is feature-local and self-contained:

```text
requirements/<classification>/<feature>/prototypes/<prototype-id>/
  prototype.json                # manifest: brief, variants, scenarios, pinned design profile
  variants/
    <variant-id>.html           # declarative, script-free HTML using the pinned token set
  companions/                   # optional `.pen` or other source artefacts
  handoff/                      # generated offline hand-off, only after review approval
```

`prototype.json` pins the design profile by id, version, and fingerprint, declares two hypothesis-led variants by default, and traces to the PRD's requirement IDs. Generated variants contain no executable JavaScript; behaviour is expressed only through the repository's declarative runtime attributes. Interactive work stays in `prototypes/`; executive and stakeholder reports stay in `reports/`.

Example fixtures use the equivalent `examples/<feature>/prototypes/<prototype-id>/` layout.
