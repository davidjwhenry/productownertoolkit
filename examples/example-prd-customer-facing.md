...
- This PRD covers a customer-facing savings feature that lets {Company XYZ} customers create named savings pots and fund them through manual transfers, recurring transfers, and card round-ups.
...
- The product outcome is a more habitual savings behavior inside the {Company XYZ} app.
...
| **The current app does not help customers build saving habits** | {Company XYZ} misses a high-frequency engagement loop tied to salary cycles and everyday spending. |
| **Savings intent is disconnected from daily banking activity** | Customers do not see progress in context, and {Company XYZ} loses the chance to make the current account feel more valuable. |
...
| **SAM** | 1.2M digitally active current-account customers | Target segment aligned to {Company XYZ}'s mobile-first audience. |
...
| Funding from external bank accounts | MVP should stay inside the {Company XYZ} account ecosystem to reduce operational and reconciliation complexity. |
...
| **Internal transfer service** | Existing service for moving funds within {Company XYZ} accounts | Used for manual funding, recurring transfers, and withdrawals. |
